...
function Sallar() {
    ...
    // Tip, renk, materyal ve desen örnekleri
    const types = ['Şal', 'Bere'];
    const colors = ['Siyah', 'Beyaz', 'Krem', 'Mavi', 'Pembe'];
    const materials = ['Pamuk', 'İpek', 'Viskon', 'Polyester'];
    const patterns = ['Düz', 'Çizgili', 'Desenli', 'Ekose'];
    ...
    const filteredProducts = products.filter(product => {
        ...
        const patternMatch = selectedPattern ? product.desen === selectedPattern : true;
        const price = parseFloat(product.fiyat.replace('₺', '').replace(',', '.'));
        const priceMatch = price >= priceRange[0] && price <= priceRange[1];
        return typeMatch && colorMatch && materialMatch && patternMatch && priceMatch;
    }).sort((a, b) => {
    ...
    return (
        <div style={{position:'relative'}}>
            <div style={{marginBottom: '24px'}}>
                <div style={{fontSize: '15px', color: '#444', marginBottom: '8px'}}>ANASAYFA &gt; ŞALLAR</div>
                <h2 className="baslik" style={{textAlign:'center', fontWeight:'bold', fontSize:'48px'}}>ŞALLAR VE BERELER</h2>
            </div>
